...
session_start();    
            ...
            $stmt = $con->prepare("select un from sheck where ids = ?" );      
            $stmt->execute(array($id));
            $rows = $stmt->fetchAll();
             $stmt1 = $con->prepare("select * from register where email = ?" );      
...
foreach ($rows as $row) {
    ...
    echo "<td>" . $row1["age"] . "</td>";
     echo "<td>" . $row1["phonenumber"] . "</td>";
      echo "<td>" . $row1["location"] . "</td>";
       echo "<td>" . $row1["nationality"] . "</td>";
